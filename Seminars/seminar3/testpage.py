...
class OperationsHelper(BasePage):
    ...
    def create_post(self, title, description, content):
        ...
        description_input = self.find_element((By.XPATH, """//*[@id="create-item"]/div/div/div[2]/div/label/span/textarea"""))
    ...
    def test_contact_us(self):
        ...
        self.find_element(TestSearchLocators.LOCATOR_OUR_NAME_FIELD).send_keys("Ulrich")
        self.find_element(TestSearchLocators.LOCATOR_EMAIL_FIELD).send_keys("Ulrich@mail.ru")
        self.find_element(TestSearchLocators.LOCATOR_MESSAGE_FIELD).send_keys("Тестовое сообщение")

        self.find_element(TestSearchLocators.LOCATOR_CONTACT_US_BTN).click()
        time.sleep(10)
        
        alert = self.driver.switch_to.alert
        assert alert.text == "Form successfully submitted", "Alert text does not match"
        alert.accept()